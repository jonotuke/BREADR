...
plotLOAF <- function(
  ...
  if ((!all(col_names_check %in% names(in_tibble)))) {
    stop(paste0(
      'Input tibble/data.frame must have at least 12 columns named: ',
  ...
  if (is.null(N)) {
    ...
    if (verbose) {
      cat(sprintf(
        'Number of pairs to plot was greater than the number of available pairs.\nPlotting first %i pairs.\n',
        N
      ))
    }
  }
  ...
  # Define class breaks
  class_breaks <- tibble::tibble(
    relationship = factor(
      c('Same_Twins', 'First_Degree', 'Second_Degree', 'Unrelated'),
      levels = c('Same_Twins', 'First_Degree', 'Second_Degree', 'Unrelated')
    ),
  ...
  gg2 <- in_tibble %>%
    ...
    ggplot2::ggplot(ggplot2::aes(
      x = forcats::fct_reorder(pair, pmr),
      y = pmr
    )) +
    ggplot2::geom_hline(
      ggplot2::aes(yintercept = meanPMR, col = relationship),
      linetype = 'dashed',
      data = class_breaks,
      linewidth = 1
    ) +
    ...
    ggplot2::geom_errorbar(ggplot2::aes(
      forcats::fct_reorder(pair, pmr),
      ymin = ymin,
      ymax = ymax
    )) +
    ggplot2::geom_point(
      ggplot2::aes(fill = relationship, shape = relationship),
      size = 2,
      show.legend = TRUE
    ) +
    ggplot2::ylab("PMR") +
...
plotSLICE <- function(
  ...
  if (!all(col_names_check %in% names(in_tibble))) {
    stop(paste0(
      'Input tibble/data.frame must have 12 columns named: ',
      paste0(col_names_check, collapse = ', '),
      '.'
    ))
  }
  ...
  distribution.tib <- tibble::tibble(
    x = rep(support, 4),
    ...
    dplyr::rowwise() %>%
    dplyr::mutate(
      y = (model == 'Same_Twins' | model == 'Second_Degree') *
        stats::dbinom(x, N, p.sim) +
  ...
  distribution.plot <- distribution.tib %>%
    ...
    ggplot2::scale_y_continuous(
      limits = c(-h, max(distribution.tib$y)),
      expand = c(0, 0)
    ) +
    ggplot2::annotate(
      geom = 'point',
    ...
    ggplot2::geom_segment(
      x = x / N - zstar * sqrt((x / N) * (1 - x / N) / N),
      xend = x / N + zstar * sqrt((x / N) * (1 - x / N) / N),
      y = -h / 2,
      yend = -h / 2
    ) +
    ggplot2::geom_segment(
    ...
    ggplot2::geom_ribbon(
      ggplot2::aes(ymin = 0, ymax = y, fill = model),
      col = 'black',
      alpha = 0.5
    ) +
    ggplot2::geom_segment(
      data = midpoint.tib,
      ggplot2::aes(x = p.sim, xend = p.sim, y = 0, yend = y * 0.99),
      linetype = 'dashed',
      col = 'black'
    )
  ...
  posterior.tibble <- tibble::tibble(
    posterior = c(
      stats::dbinom(x, N, (1 - 0.5^c(1:3)) * M),
      weightedBinom(x, N, M)
    ) *
      class_prior /
      sum(
        c(stats::dbinom(x, N, (1 - 0.5^c(1:3)) * M), weightedBinom(x, N, M)) *
          class_prior
      ),
...
test_degree <- function(in_tibble, row, degree, verbose = TRUE) {
  ...
  if (verbose) {
    cat(sprintf(
      'Testing H0       : "%s" are %s-degree relatives.\n',
      pair,
      degree_string
    ))
