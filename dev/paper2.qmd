...
$$
p_k = \bar{p}\left(1 - \frac{1}{2^{k+1}} \right).
$$
...
$$
    L\left( K=k \big| X_{ij}, N_{ij} \right) = \binom{N_{ij}}{X_{ij}} p_k^{X_{ij}}(1-p_k)^{N_{ij}-X_{ij}}.
$$
...
&= \sum_{k=3}^{\infty} \binom{N_{ij}}{X_{ij}} p_k^{X_{ij}}(1-p_k)^{N_{ij}-X_{ij}} f_{\lambda}(k),
\end{aligned}
$$
...
$$
P\left( K=k \big| X_{ij}, N_{ij} \right) = \frac{L\left( K=k \big| X_{ij}, N_{ij} \right)}{\left[ \sum_{\ell=0}^{2} L\left( K=\ell \big| X_{ij}, N_{ij} \right) \right] + L\left( K\geq 3 \big| X_{ij}, N_{ij} \right)}
$$

for which the denominator equals one, by construction. Hence,

$$
    P\left( K=k \big| X_{ij}, N_{ij} \right) =L\left( K=k \big| X_{ij}, N_{ij} \right)P\left(K=k\right).
$$
